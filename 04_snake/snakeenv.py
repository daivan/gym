...
def snake_generating(SnakeList, SnakeDir):
    ...
    if len(SnakeDir) == 0:
        ...
        if SnakeList[0][0] > SnakeList[1][1]:
            dir_list.remove('up')
        if SnakeList[0][0] < SnakeList[1][0]:
...
class SnakeEnv(gym.Env):
    ...
    def __init__(self, render_mode = None):
        ...
        self.observation_space = spaces.Box(low=0, high=40, shape=(5, ), dtype=np.int64)


    def step(self, action):

        self.snake_eat, self.snake_dead, self.score, self.apple_pos = collision(self.snake_list, self.apple_pos, self.snake_dir, self.snake_eat, self.snake_dead, self.score)
        self.snake_list, self.snake_eat = updating_snake(self.snake_dir, self.snake_list, self.snake_eat, self.snake_dead)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.observation = [self.snake_list[0][0], self.snake_list[0][1], self.apple_pos[0], self.apple_pos[1], self.numerical_dir]                   
        self.observation = np.array(self.observation)
    ...
    def render(self, render_mode='human'):
        ...
        # snake head
        pygame.draw.rect(self.display, 'WHITE', (self.snake_list[0][0] * 15 + 1, self.snake_list[0][1] * 15 + 1, 13, 13))
